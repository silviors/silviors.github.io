...
int emailalarme = 14, emailinterfone = 15, campainha = 54, emailcerca = 18, buzina = 17, relealarme = 16, rftrans = 19, hora01 = 20, hora02 = 21;
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
int  controleportao = 1, ctltimer = 1, minuto = 0, minutoportao = 0, sensorLDR = A1, sensorAlarme = A9; //controle timer jardim e hora portao
...
void setup() {
  ...
  server.begin();
  vw_set_tx_pin(19);//rftransmissor
...
void loop()
{
  ...
  if (inthora >= horanoite && luminosidade < 75 && ctltimer > 0) {
...
void enviorf(int valorlido) {
  ...
  String logacoes[] = {"DISPARO ALARME", "ACIONOU PORTAO ", "LUZ FRENTE PORT", "LUZ GARAGEM    ", "LUZ HALL ENTRAD", "LUZ SALA DE TV ",
                       ...
                       "LUZ REFLE MURO", "LUZ PORAO EDICU", "DICROICAS SALA ", "TIMER JARDIM   ", "TOQUE CAMPAINHA"
                      };
...
void envialog()
{
  ...
  if (client.connect(serversmtp, 2525))  //Porta de conexao do servidor do email.
  {
    ...
    recebe();
    delay(timer);
    client.println("Subject: Logs de eventos do arduino");  //Assunto
    recebe();
    delay(timer);
    for (int i = 0; logs[i]; i++) {
      client.println(logs[i]);
