...
String stringrecebidoRF = "NADA", strdata;
...
unsigned int dlogs[tamlog], hlogs[tamlog], alogs[tamlog], itamlog = 0;
...
void loop()
{
  ...
  strdata = "1" + convertestr(dataehora.date) + convertestr(dataehora.mon) + String(dataehora.year - 2000); //995183 995183
  ...
  if ( (dataehora.min) != minuto) { //envio do horario via rf e timer
    ...
    if (minuto < 2) enviostring(strdata);
    ...
    while (client.connected())
    {
      if (client.available())
      {
        ...
        if (c == 'n')
        {
          ...
          client.println("<br><br>Ardinoprincipalv20</div>");
...
void envia()
{
  ...
  if (client.connect(serversmtp, 2525))  //Porta de conexao do servidor do email.
  {
    ...
    recebe();
    delay(timer);
    if (digitalRead(emailalarme) != LOW) {
...
void enviostring(String strenvio) {
  char string_CharMsg_envio[9];
  for (int i = 0; i < strenvio.length(); i++) string_CharMsg_envio[i] = char(strenvio[i]);
  string_CharMsg_envio[strenvio.length()] = '\0';
  for (int i = 0; i < repetenvio; i++) {
    vw_send((uint8_t *)string_CharMsg_envio, strlen(string_CharMsg_envio));    //Envio dos dados
    vw_wait_tx();//Aguarda envio dos dados
  }
}
